...
const ArticlesPage = () => {
    ...
    function switchToListView() {
      ...
      articleCards.forEach((card) => {
        ...
        card.classList.add(
          ...
          "h-auto",
          "max-w-full"
        ); // ✅ Ensures mobile and desktop layout
  ...
  const filteredArticles = articles.filter((article) => {
    return (
      (selectedFilters.writer === "" ||
        article.writer === selectedFilters.writer) &&
      (selectedFilters.translator === "" ||
        article.translator === selectedFilters.translator) &&
      (selectedFilters.language === "" ||
        article.language === selectedFilters.language) &&
                  ...
                  >
                    ...
                    {writers.map((writer) => (
                      <option
                        key={writer._id}
                        value={writer.name}
                        className="cursor-pointer text-slate-700 gulzartext"
                      >
                        {writer.name}
                      </option>
                    ))}
                  ...
                  >
                    ...
                    {topics.map((topic) => (
                      <option
                        key={topic._id}
                        value={topic.topic}
                        className="text-slate-700 gulzartext"
                      >
                        {topic.topic}
                      </option>
                    ))}
